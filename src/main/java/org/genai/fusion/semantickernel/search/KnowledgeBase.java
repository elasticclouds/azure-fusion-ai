package org.genai.fusion.semantickernel.search;
...
@Component
public class KnowledgeBase implements ApplicationListener<ContextRefreshedEvent> {

    Logger log = LoggerFactory.getLogger(KnowledgeBase.class);
    ...
    public KnowledgeBase(OpenAITextEmbeddingGenerationService embeddingGenerationService, VectorStoreRecordCollection<String, Document> inMemoryVectorStore) {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        try {
            ...
            savedCollection.subscribe(
                    list -> log.info("Knowledge Base indexed successfully"),
                    error -> log.error("Error indexing Knowledge Base", error),
                    () -> log.info("Indexing process completed")
            );

        } catch (IOException e) {
            log.error("Could not read the Knowledge Base file", e);
        }
    }
