...
public class SKAssistant {
    ...
    private final ContextVariableTypeConverter<String> stringContextVariableTypeConverter;
    private final ContextVariableTypeConverter<LoanApplication> loanApplicationContextVariableTypeConverter;


    private InvocationContext invocationContext;
    private Map<String, SKChatManager> chatsInMemory;

    public SKAssistant(Kernel kernel, ContextVariableTypeConverter<String> stringContextVariableTypeConverter, ContextVariableTypeConverter<LoanApplication> loanApplicationContextVariableTypeConverter) {
        this.kernel = kernel;
        this.stringContextVariableTypeConverter = stringContextVariableTypeConverter;
        this.loanApplicationContextVariableTypeConverter = loanApplicationContextVariableTypeConverter;
        this.chatsInMemory = new HashMap<>();
        // Allowing LLM to find and invoke the right function(s) required to perform a
        // task
        this.invocationContext = InvocationContext.builder()
                .withToolCallBehavior(ToolCallBehavior.allowAllKernelFunctions(true))
                .withReturnMode(InvocationReturnMode.LAST_MESSAGE_ONLY)
                .build();
        // Setting the type converter globally, so when a String object
        // encountered,
        // this can be used to convert the object to string and vice-versa
        ContextVariableTypes.addGlobalConverter(stringContextVariableTypeConverter);
        ContextVariableTypes.addGlobalConverter(loanApplicationContextVariableTypeConverter);
    }
    ...
    public Flux<String> chat(String chatId, String userMessage) throws ServiceNotFoundException {
        ...
        return chatMessageContentsAsync.flatMapIterable(list -> list.stream().map(content -> {
